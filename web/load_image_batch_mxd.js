...
const NODE_TYPES = new Set(["Load Image Batch MXD"]);

// Cache the folder lists for both sources so flipping the "source" widget is
// instant and never needs a page refresh.
let _foldersPromise = null;
function fetchFolders() {
    if (!_foldersPromise) {
        _foldersPromise = api
            .fetchApi("/mxd/image_batch/folders")
            .then((resp) => resp.json())
            .catch((err) => {
                console.error("[LoadImageBatchMXD] Failed to fetch folders:", err);
...
app.registerExtension({
    name: "ComfyUI-MaxedOut.LoadImageBatchMXD",
