...
class MxdLtx2PowerLoraLoader extends MxdBaseServerNode {
  ...
  constructor(title = NODE_CLASS.title) {
    ...
    this.logger = mxdRuntime.newLogSession("[LTX2 Power Lora Loader]");
  ...
  configure(info) {
    ...
    for (const widgetValue of info.widgets_values || []) {
      ...
      } else if (widgetValue?.type === "Ltx2StrengthWidget" && widgetValue?.key) {
        savedAdvanced[widgetValue.key] = widgetValue.value;
      }
    }

    this.addNonLoraWidgets();

    for (const [key, value] of Object.entries(savedAdvanced)) {
  ...
  addNewLoraWidget(lora) {
    ...
    const widget = this.addCustomWidget(new Ltx2PowerLoraLoaderWidget("lora_" + this.loraWidgetsCounter));
...
class Ltx2PowerLoraLoaderWidget extends MxdBaseWidget {
  constructor(name) {
    super(name);
    this.type = "custom";
